...
**Supplementary Figure S1. Interval calibration behavior of the verified legacy Random Forest baseline.** Nominal coverage is compared with empirical coverage for both the raw tree-quantile intervals and an OOB-based split-conformal layer built from a deterministic GDSC reference partition. The panels show that the calibration workflow is reproducible and benchmark-backed, but that it still under-covers the external CCLE benchmark and does not yet solve the cross-domain reliability gap.
