...
# Figure 3: Simple Network Visualization
def create_network_visualization():
    """Create a simple network visualization without networkx"""
    fig, ax = plt.subplots(1, 1, figsize=(12, 10))
    
    # Define node positions manually
    drugs_pos = [(1, i*1.5 + 2) for i in range(6)]
    cells_pos = [(5, i*1.2 + 1.5) for i in range(8)]
    targets_pos = [(9, i*2 + 2) for i in range(5)]
    
    # Draw nodes
    for pos in drugs_pos:
        circle = plt.Circle(pos, 0.3, color='#FF6B6B', alpha=0.8)
        ax.add_patch(circle)
        ax.text(pos[0], pos[1], f'D{drugs_pos.index(pos)+1}', ha='center', va='center', fontsize=8)
    
    for pos in cells_pos:
        circle = plt.Circle(pos, 0.25, color='#4ECDC4', alpha=0.8)
        ax.add_patch(circle)
        ax.text(pos[0], pos[1], f'C{cells_pos.index(pos)+1}', ha='center', va='center', fontsize=8)
    
    for pos in targets_pos:
        circle = plt.Circle(pos, 0.35, color='#45B7D1', alpha=0.8)
        ax.add_patch(circle)
        ax.text(pos[0], pos[1], f'T{targets_pos.index(pos)+1}', ha='center', va='center', fontsize=8)
    
    # Draw some sample connections
    np.random.seed(42)
    for i, drug_pos in enumerate(drugs_pos[:4]):
        # Connect to random cells
        selected_cells = np.random.choice(len(cells_pos), size=3, replace=False)
        for cell_idx in selected_cells:
            cell_pos = cells_pos[cell_idx]
            ax.plot([drug_pos[0], cell_pos[0]], [drug_pos[1], cell_pos[1]], 
                   'gray', alpha=0.3, linewidth=1)
        
        # Connect to random targets
        selected_targets = np.random.choice(len(targets_pos), size=2, replace=False)
        for target_idx in selected_targets:
            target_pos = targets_pos[target_idx]
            ax.plot([drug_pos[0], target_pos[0]], [drug_pos[1], target_pos[1]], 
                   'gray', alpha=0.3, linewidth=1)
    
    # Add layer labels
    ax.text(1, 0.5, 'DRUGS', fontsize=14, fontweight='bold', ha='center')
    ax.text(5, 0.5, 'CELL LINES', fontsize=14, fontweight='bold', ha='center')
    ax.text(9, 0.5, 'TARGETS', fontsize=14, fontweight='bold', ha='center')
